...
This is a monorepo project. Key languages: TypeScript (primary). Schema validation uses Zod. ORM is Drizzle (PostgreSQL). Auth uses Better Auth. Package manager: Bun. Monorepo orchestration: Turbo.

## Project Map

```text
nexa/                              # Bun monorepo, Turbo orchestration
├── apps/
│   ├── server/                    # @nexa/server — Elysia API (port 3000)
│   │   ├── src/index.ts           # App entry: Elysia + CORS + OpenAPI + error handler + module routes
│   │   ├── src/container.ts       # tsyringe DI container (Database, CacheClient, Auth, repos, gateways)
│   │   ├── src/modules/
...
│   │   │   │   └── middleware/    # error-handler.ts (Elysia onError hook)
...
│   ├── db/                        # @nexa/db — Drizzle ORM (PostgreSQL via postgres.js)
│   │   └── src/schema/            # Tables: product, user-permission, auth (Better Auth tables), relations
│   ├── env/                       # @nexa/env — t3-env typed env vars (server, admin)
...
- Server tests: `cd apps/server && bun run test`
...
- admin has no unit tests currently — only server tests are tracked.
...
- **PostgreSQL upsert**: Use `.onConflictDoUpdate({ target: ..., set: ... })` — `onDuplicateKeyUpdate` is MySQL-only.
- **bun run test vs bun test**: Use `bun run test` (runs vitest, excludes integration tests). `bun test` uses bun's native runner and also runs integration tests that need a live database.
...
- `bun run dev:server` / `bun run dev:admin` — Start individual apps
- `cd apps/server && bun run test` — Run server unit tests (vitest, excludes integration tests)
- `cd apps/server && bunx tsgo --noEmit` — Typecheck server
