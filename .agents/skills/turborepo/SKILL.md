...
{ "scripts": { "build": "bunx tsgo", "lint": "eslint .", "test": "vitest" } }
...
{
  "scripts": {
    "build": "cd apps/web && next build && cd ../api && bunx tsgo",
...
1. Read the package's script (e.g., `"build": "bunx tsgo"`, `"test": "vitest"`)
...
- Vite/Rollup: `["dist/**"]`
- bunx tsgo: `["dist/**"]` or custom `outDir`

**TypeScript `--noEmit` can still produce cache files:**

When `incremental: true` in tsconfig.json, `bunx tsgo --noEmit` writes `.tsbuildinfo` files even without emitting JS. Check the tsconfig before assuming no outputs:

```json
// If tsconfig has incremental: true, bunx tsgo --noEmit produces cache files
{
  "tasks": {
    "typecheck": {
...
3. If no incremental mode, `bunx tsgo --noEmit` produces no files
...
- **Packages** (e.g., `@acme/db`, `@acme/validators`) have `"dev": "bunx tsgo"` — one-shot type generation that completes quickly
